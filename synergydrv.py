...
def synergydrv(datain,outputfile,dataanno,iternum=0, limitinter=5):
	ii = 0
	proteinname={}
...
def synergydrv2nd(datain,dataresult,outputfile,dataanno,iternum=0):
	ii = 0
	proteinname=[]
	with open(dataanno, "r") as f:
		for line in f.readlines():
	...
	for smallselect in selectcols:
		...
		if (np.sum(select) < len(smallselect)):
			...
			oneout = synergyconselect(X0[:,select],y,selectcol,iternum)
...
def synergyconselect(X0,y,proteinnames,iternum=0):
	ii = 0
	proteinname={}
